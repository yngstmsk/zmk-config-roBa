...
CONFIG_ZMK_SLEEP=n                  # ディープスリープ無効化（BLE切断防止）
```
...
CONFIG_BT_MAX_PAIRED=3              # MAX_CONNと同値必須（ZMK公式ドキュメント要件）
CONFIG_ZMK_SLEEP=n                  # ディープスリープ無効化（BLE切断防止）
CONFIG_ZMK_BLE_THREAD_STACK_SIZE=1024  # BLEスレッドスタック拡張（デフォルト512）
CONFIG_ZMK_BLE_KEYBOARD_REPORT_QUEUE_SIZE=30  # HIDキューサイズ拡張（デフォルト20）
```
...
| `CONFIG_BT_MAX_PAIRED=3` | roBa_R | ZMK公式ドキュメントでMAX_CONNと同値必須と明記。不整合がボンドテーブル管理エラーを引き起こす |
...
| `CONFIG_ZMK_SLEEP=n` | 両側 | ディープスリープを明示的に無効化。スリープによるBLE切断を防止 |
| `CONFIG_ZMK_BLE_THREAD_STACK_SIZE=1024` | roBa_R | BLEスレッドスタックを拡張（デフォルト512）。キーボード+マウス+バッテリー+スプリット同時処理でのスタックオーバーフロー対策 |
| `CONFIG_ZMK_BLE_KEYBOARD_REPORT_QUEUE_SIZE=30` | roBa_R | HIDキューを拡張（デフォルト20）。バースト入力時のキュー溢れ対策 |
